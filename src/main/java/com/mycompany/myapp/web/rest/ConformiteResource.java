...
public class ConformiteResource {
     ...
     */
    @GetMapping("")
    public List<Conformite> getAllConformites(@RequestParam(name = "filter", required = false) String filter) {
        if ("parametrage-is-null".equals(filter)) {
            log.debug("REST request to get all Conformites where parametrage is null");
            return StreamSupport
                .stream(conformiteRepository.findAll().spliterator(), false)
                .filter(conformite -> conformite.getParametrage() == null)
                .toList();
        }
