...
public class DemandeResource {
     ...
     */
    @GetMapping("")
    public List<Demande> getAllDemandes(@RequestParam(name = "filter", required = false) String filter) {
        if ("parametrage-is-null".equals(filter)) {
            log.debug("REST request to get all Demandes where parametrage is null");
            return StreamSupport
                .stream(demandeRepository.findAll().spliterator(), false)
                .filter(demande -> demande.getParametrage() == null)
                .toList();
        }
