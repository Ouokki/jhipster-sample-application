...
public class TarifResource {
    ...
    public TarifResource(TarifRepository tarifRepository) {
    ...
    public ResponseEntity<Tarif> partialUpdateTarif(@PathVariable(value = "id", required = false) final Long id, @RequestBody Tarif tarif)
        ...
        Optional<Tarif> result = tarifRepository
            .findById(tarif.getId())
            .map(existingTarif -> {
                if (tarif.getTarifParDefaut() != null) {
                    existingTarif.setTarifParDefaut(tarif.getTarifParDefaut());
                }
                if (tarif.getTarifMinimum() != null) {
                    existingTarif.setTarifMinimum(tarif.getTarifMinimum());
                }
                if (tarif.getTarifMaximum() != null) {
                    existingTarif.setTarifMaximum(tarif.getTarifMaximum());
                }
                if (tarif.getUnite() != null) {
     ...
     */
    @GetMapping("")
    public List<Tarif> getAllTarifs(@RequestParam(name = "filter", required = false) String filter) {
        if ("tarifcommercant-is-null".equals(filter)) {
            log.debug("REST request to get all Tarifs where tarifCommercant is null");
            return StreamSupport
                .stream(tarifRepository.findAll().spliterator(), false)
                .filter(tarif -> tarif.getTarifCommercant() == null)
                .toList();
        }

        if ("optionproduitcommerces-is-null".equals(filter)) {
            log.debug("REST request to get all Tarifs where optionProduitCommerces is null");
            return StreamSupport
                .stream(tarifRepository.findAll().spliterator(), false)
                .filter(tarif -> tarif.getOptionProduitCommerces() == null)
                .toList();
        }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<Tarif> getTarif(@PathVariable("id") Long id) {
