...
public class TarifReferenceOptionResource {
    ...
    public List<TarifReferenceOption> getAllTarifReferenceOptions(
        ...
        if ("tarif-is-null".equals(filter)) {
            log.debug("REST request to get all TarifReferenceOptions where tarif is null");
            return StreamSupport
                .stream(tarifReferenceOptionRepository.findAll().spliterator(), false)
                .filter(tarifReferenceOption -> tarifReferenceOption.getTarif() == null)
                .toList();
        }
