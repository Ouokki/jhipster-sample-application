...
public class OffreProduit implements Serializable {
    ...
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(value = { "referentielCR", "offreProduit" }, allowSetters = true)
    ...
    public void setCrs(Set<CR> cRS) {
        if (this.crs != null) {
            this.crs.forEach(i -> i.setOffreProduit(null));
        }
        if (cRS != null) {
            cRS.forEach(i -> i.setOffreProduit(this));
        }
        this.crs = cRS;
    }

    public OffreProduit crs(Set<CR> cRS) {
        this.setCrs(cRS);
        return this;
    }

    public OffreProduit addCr(CR cR) {
        this.crs.add(cR);
        cR.setOffreProduit(this);
        return this;
    }

    public OffreProduit removeCr(CR cR) {
        this.crs.remove(cR);
        cR.setOffreProduit(null);
        return this;
    }
