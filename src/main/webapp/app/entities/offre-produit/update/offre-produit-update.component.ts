...
export class OffreProduitUpdateComponent implements OnInit {
  ...
  protected updateForm(offreProduit: IOffreProduit): void {
    ...
    this.cRSSharedCollection = this.cRService.addCRToCollectionIfMissing<ICR>(this.cRSSharedCollection, offreProduit.cr);
    this.offresSharedCollection = this.offreService.addOffreToCollectionIfMissing<IOffre>(this.offresSharedCollection, offreProduit.offre);
    this.produitsSharedCollection = this.produitService.addProduitToCollectionIfMissing<IProduit>(
      this.produitsSharedCollection,
      offreProduit.produit,
    );
  }
  ...
  protected loadRelationshipsOptions(): void {
    ...
    this.cRService
      ...
      .pipe(map((cRS: ICR[]) => this.cRService.addCRToCollectionIfMissing<ICR>(cRS, this.offreProduit?.cr)))
      .subscribe((cRS: ICR[]) => (this.cRSSharedCollection = cRS));

    this.offreService
      .query()
      .pipe(map((res: HttpResponse<IOffre[]>) => res.body ?? []))
      .pipe(map((offres: IOffre[]) => this.offreService.addOffreToCollectionIfMissing<IOffre>(offres, this.offreProduit?.offre)))
    ...
    this.produitService
      ...
      .pipe(
        map((produits: IProduit[]) => this.produitService.addProduitToCollectionIfMissing<IProduit>(produits, this.offreProduit?.produit)),
      )
