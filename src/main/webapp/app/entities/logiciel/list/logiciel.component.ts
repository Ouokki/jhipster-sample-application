...
import { EntityArrayResponseType, LogicielService } from '../service/logiciel.service';
...
export class LogicielComponent implements OnInit {
  ...
  trackId = (_index: number, item: ILogiciel): number => this.logicielService.getLogicielIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.logiciels = this.refineData(dataFromBody);
  }

  protected refineData(data: ILogiciel[]): ILogiciel[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ILogiciel[] | null): ILogiciel[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.logicielService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
