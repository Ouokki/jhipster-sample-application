import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRoute } from '@angular/router';
import { RouterTestingModule } from '@angular/router/testing';
import { of } from 'rxjs';

import { ParametrageService } from '../service/parametrage.service';

import { ParametrageComponent } from './parametrage.component';

describe('Parametrage Management Component', () => {
  let comp: ParametrageComponent;
  let fixture: ComponentFixture<ParametrageComponent>;
  let service: ParametrageService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [
        RouterTestingModule.withRoutes([{ path: 'parametrage', component: ParametrageComponent }]),
        HttpClientTestingModule,
        ParametrageComponent,
      ],
      providers: [
        {
          provide: ActivatedRoute,
          useValue: {
            data: of({
              defaultSort: 'id,asc',
            }),
            queryParamMap: of(
              jest.requireActual('@angular/router').convertToParamMap({
                page: '1',
                size: '1',
                sort: 'id,desc',
              }),
            ),
            snapshot: { queryParams: {} },
          },
        },
      ],
    })
      .overrideTemplate(ParametrageComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(ParametrageComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(ParametrageService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 123 }],
          headers,
        }),
      ),
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.parametrages?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to parametrageService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getParametrageIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getParametrageIdentifier).toHaveBeenCalledWith(entity);
      expect(id).toBe(entity.id);
    });
  });
});
