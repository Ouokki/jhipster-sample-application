...
import { EntityArrayResponseType, GarantieService } from '../service/garantie.service';
...
export class GarantieComponent implements OnInit {
  ...
  trackId = (_index: number, item: IGarantie): number => this.garantieService.getGarantieIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.garanties = this.refineData(dataFromBody);
  }

  protected refineData(data: IGarantie[]): IGarantie[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IGarantie[] | null): IGarantie[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.garantieService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
