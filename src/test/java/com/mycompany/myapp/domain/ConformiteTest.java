...
class ConformiteTest {
    ...
    void parametrageTest() throws Exception {
        ...
        conformite.addParametrage(parametrageBack);
        assertThat(conformite.getParametrages()).containsOnly(parametrageBack);
        assertThat(parametrageBack.getConformite()).isEqualTo(conformite);

        conformite.removeParametrage(parametrageBack);
        assertThat(conformite.getParametrages()).doesNotContain(parametrageBack);
        assertThat(parametrageBack.getConformite()).isNull();

        conformite.parametrages(new HashSet<>(Set.of(parametrageBack)));
        assertThat(conformite.getParametrages()).containsOnly(parametrageBack);
        assertThat(parametrageBack.getConformite()).isEqualTo(conformite);

        conformite.setParametrages(new HashSet<>());
        assertThat(conformite.getParametrages()).doesNotContain(parametrageBack);
        assertThat(parametrageBack.getConformite()).isNull();
    }
}
